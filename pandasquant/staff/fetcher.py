...
class Databaser(object):
    ...
    def trade_date(start: Union[datetime.date, datetime.datetime, str],
        ...
        data = pd.read_sql(f'select trade_date from trade_date_{freq} ' + \
            ...
            f'and trade_date <= "{end}"', stock)
        data = data.trade_date.sort_values().tolist()
        return data

    def trade_date_w(start_date: Union[str, datetime.datetime],
                    end_date: Union[str, datetime.datetime],
                    weekday: int) -> list[datetime.date]:
      '''Update data for table stock.trad_date_weekly
      -----------------------------------------------
    ...
    def trade_date_m(start_date: Union[str, datetime.datetime],
                    end_date: Union[str, datetime.datetime]) -> list[datetime.date]:
    ...
    def market_daily_single(start_date: Union[datetime.date, str, datetime.datetime],
                        end_date: Union[datetime.date, str, datetime.datetime],
                        code: str
                        ) -> pd.DataFrame:
        """
        获取某只股票某一时段的行情
